...
class UserAPITests(TestCase):
    def setUp(self):
        ...
        self.wrong_user_data = {"username": "testuser", "password": "wrongpassword"}
    ...
    def _get_token_for_user(self, user_payload):
        ...
        self.assertEqual(
            ...
            201,
            f"Failed to register user for token retrieval. Response: {response.content.decode()}",
        )
    ...
    def test_register_success(self):
        ...
        self.assertTrue(
            AuthToken.objects.filter(user__username=self.user_data["username"]).exists()
        )
        token_in_db = AuthToken.objects.get(
    ...
    def test_login_success(self):
        ...
        self.assertTrue(
            AuthToken.objects.filter(
                user__username=self.user_data["username"], key=login_data["token"]
            ).exists()
        )
    ...
    def test_login_wrong_password(self):
        ...
        response = self.client.post(
            self.login_url,
            data=json.dumps(self.wrong_user_data),
            content_type="application/json",
        )
    ...
    def test_me_endpoint_with_valid_token(self):
        ...
        self.assertEqual(
            response.status_code, 200, f"Response content: {response.content.decode()}"
        )
        response_data = response.json()
