...
class UserRegistrationLoginTests(APITestCase):
    def setUp(self):
        ...
        conflict_reg_username = (
            f"testuser_conflict_reg_{User.objects.count() + 800}"  # Увеличим счетчик
        )
        User.objects.create_user(
            username=conflict_reg_username, password="somepassword"
        )
        ...
        self.login_test_username = (
            f"testuser_login_{User.objects.count() + 80}"  # Увеличим счетчик
        )
        self.login_test_password = "login_password_123"
        User.objects.create_user(
            username=self.login_test_username, password=self.login_test_password
        )
    ...
    def test_registration_username_conflict(self):
        ...
        response = self.client.post(
            self.register_url, self.user_data_conflict, format="json"
        )
        self.assertEqual(
    ...
    def test_login_wrong_password(self):
        ...
        login_data = {"username": self.login_test_username, "password": "wrongpassword"}
        response = self.client.post(self.login_url, login_data, format="json")
        self.assertEqual(
    ...
    def test_registration_invalid_data_empty(self):
        ...
        self.assertEqual(
            response.status_code, status.HTTP_422_UNPROCESSABLE_ENTITY, response.content
        )
        response_data = response.json()
        self.assertIn("detail", response_data)
        ...
        has_username_error = any(
            err.get("loc") == ["body", "payload", "username"]
            for err in response_data["detail"]
        )
        has_password_error = any(
            err.get("loc") == ["body", "payload", "password"]
            for err in response_data["detail"]
        )

        self.assertTrue(
            has_username_error, "Ошибка для 'username' не найдена в деталях."
        )
        # Пароль также должен иметь ошибку (пустая строка короче 8 символов)
        self.assertTrue(
            has_password_error, "Ошибка для 'password' не найдена в деталях."
        )
    ...
    def test_registration_invalid_data_short_password(self):
        ...
        for error in response_data["detail"]:
            if error.get("loc") == ["body", "payload", "password"]:
                ...
                self.assertIn(
                    "String should have at least 8 characters", error.get("msg", "")
                )
                break
        self.assertTrue(
            password_error_found, "Ошибка валидации длины пароля не найдена."
        )
